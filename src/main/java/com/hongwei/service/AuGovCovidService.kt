        ...
        for (i in 0..inDays) {
            ...
            if (recordsByDay.isNotEmpty()) {
                val dayData = AusDataByStatePerDay(
                        ...
                        caseInStIves = recordsByDay.filter { it.postcode == 2075L }.size.toLong(),
                        caseInTurramurra = recordsByDay.filter { it.postcode == 2074L }.size.toLong(),
                        caseInHornsby = recordsByDay.filter { it.postcode == 2077L }.size.toLong(),
                        caseInEpping = recordsByDay.filter { it.postcode == 2121L }.size.toLong(),
                        caseInCarlingford = recordsByDay.filter { it.postcode == 2118L }.size.toLong(),
                        caseInMacquarie = recordsByDay.filter { it.postcode == 2113L }.size.toLong()
                )
                result.add(dayData)
            }
        }
